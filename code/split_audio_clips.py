...
subj_ids = [i[5:8] for i in list(filter(lambda v: re.match(f"^CONV_1[0-9][0-9]*", v), os.listdir(path_audio_raw)))]
...
def original_code():
    ...
    for subj_name in subj_ids:
        try:
            # find csv
            subj_csv_filenames = list(filter(lambda v: re.match(f"^CONV_{subj_name}.*.csv$", v), os.listdir(path_csv)))

            # if there's only one csv file for the subject, proceed
            if len(subj_csv_filenames) == 1:

                subj_csv_filepath = os.path.join(path_csv, list(filter(lambda v: re.match(f"^CONV_{subj_name}.*.csv$", v), os.listdir(path_csv)))[-1])

                # extract information from csv
                subj_csv = pd.read_csv(subj_csv_filepath)

                # find timingslog
                subj_timingslog_filepath = os.path.join(path_timingslog, list(filter(lambda v: re.match(f"^CONV_{subj_name}_TimingsLog.*.csv$", v), os.listdir(path_timingslog)))[-1])
                # extract information from csv
                ...
                subj_timingslog = subj_timingslog[subj_timingslog['role'] == 'trial_intro']
                ...
                audio_positions = subj_timingslog['audio_position'].reset_index(drop=True)
                    
                # find raw audio clip
                subj_audio_filepath = os.path.join(path_audio_raw, list(filter(lambda v: re.match(f"^CONV_{subj_name}.*RecordedAudio*", v), os.listdir(path_audio_raw)))[-1])
                ...
                for i in range(len(trials)):
                    ...
                    new_clip_filepath = os.path.join(new_clip_dir, f"conv-{subj_name}_run-{runs[i]}_set-{sets[i]}_trial-{trials[i]}_item-{items[i]}_condition-{conditions[i]}_first-{first_speakers[i]}.wav")
                    ...
                    wavmaker(subj_audio_filepath, new_clip_filepath, channel, rate, trial_start, trial_end)
